...
class ScriptCompiler:
    ...
    def __init__(self, vm):
        ...
        logger.debug(f"{len(self._vm.colors)} web colors")
    ...
    def compile(self, script_file):
        ...
        if self._do_for >= 0:
            logger.debug(f"{self._do_for + 1} do-for statement(s) open at script end")

        sf.close()

        # End of main file
        if self._file_depth == 0:
            logger.debug(f"{len(self._vm.stmts)} statements compiled")
        return valid
    ...
    def do_for_stmt(self, tokens):
        ...
        try:
            duration_struct = str_parse_time(tokens[1])
        except Exception as ex:
    ...
    def do_at_stmt(self, tokens):
        ...
        try:
            start_time_struct = str_parse_time(tokens[1])
        except Exception as ex:
    ...
    def do_until_stmt(self, tokens):
        ...
        try:
            start_time = str_parse_time(tokens[1])
        except Exception as ex:
    ...
    def pause_stmt(self, tokens):
        ...
        try:
            pause_struct = str_parse_time(tokens[1])
        except Exception as ex:

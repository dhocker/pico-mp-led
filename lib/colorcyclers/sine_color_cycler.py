...
class SineColorCycler:
    ...
    def to_tkcolor_list(color_list):
        ...
        for i in range(len(color_list)):
            ...
            rgb = SineColorCycler.decode_rgb(color_list[i])
            tkcolor_list.append("#%02x%02x%02x" % rgb)
        return tkcolor_list

    def __calculate_next_color(self, cycle):
        red = SineColorCycler.__calc_color_component(cycle, self.red_freq, self.red_phase, self.width, self.center)
        green = SineColorCycler.__calc_color_component(cycle, self.green_freq, self.green_phase, self.width, self.center)
        blue = SineColorCycler.__calc_color_component(cycle, self.blue_freq, self.blue_phase, self.width, self.center)
    ...
    def print_colors(self):
        ...
        for i in range(self.colors):
            ...
            rgb = SineColorCycler.decode_rgb(color)
            print(i, rgb)
...
if __name__ == "__main__":
    ...
    color_generator = SineColorCycler()
